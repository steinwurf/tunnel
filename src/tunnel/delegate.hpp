...
public:
    ...
    delegate(F&& f)
    {
        static_assert(sizeof(std::decay_t<F>) <= MaxBytes,
                      "Delegate closure F is too damn big");
    ...
    /// Construct a delegate from a pointer to a function argument
    delegate(R (*const func)(Args...)) :
        delegate([func](Args... args)
                 { return func(std::forward<Args>(args)...); })
    {
        assert(func != nullptr && "Delegate function pointer is null");
    }

    /// Construct a delegate to a member function in class C
    /// @param c Pointer to object of type C
    /// @param func Pointer to a member function
    template <class C>
    delegate(std::decay_t<C>* const c, R (C::*const func)(Args...)) :
        delegate([c, func](Args... args)
    ...
    /// Construct a delegate to a const member function in class C
    ...
    /// Construct a delegate from an existing invoke function and constructed
    /// function object
    ...
    /// Call the operator for invoking the delegate
